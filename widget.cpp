...
void Widget::initHedgedWidget()
{
    ...
    ui->chooseStartDate->setDate (QDate::currentDate().addDays(-365*1));
    ui->chooseEndDate->setDate (QDate::currentDate().addDays(-3));
...
{
    if (m_portfolioMap.size () == 0)
    {
        QMessageBox::critical(NULL, "Error", QString("还未选择策略"));
        return;
    }
    else if (m_portfolioMap.find("Error") != m_portfolioMap.end())
    {
        QMessageBox::critical(NULL, "Error", QString(QString("读取策略: %1, 出错")).arg(m_portfolioFileName));
    ...
    else
    {
        ...
        if (QMessageBox::Yes == QMessageBox::warning(this, "确认信息", info, QMessageBox::Yes|QMessageBox::No))
        {
            ...
            QWidget* chartView = new ChartForm(0, ui->programInfo_tableView,
                                               ...
                                               m_portfolioMap, m_portfolioFileName,
...
void Widget::on_realTimeHedgeIndexChart_clicked()
{
    if (m_portfolioMap.size () == 0) {
        ...
        updateProgramInfo (ui->programInfo_tableView, QString(QString("策略名称: %1, 策略中股票数目为: %2"))
                                                      .arg(m_portfolioFileName).arg(m_portfolioMap.size ()));
...
void Widget::on_showAVEEnergy_pushButton_clicked()
{
    ...
    QString dbhost = ui->dataSource_ComboBox->currentText();
    QString selectCode = ui->energySelectCode_lineEdit->text();
    QString hedgedCode =ui->energyHedgedCode_lineEdit->text();
    ...
    {
        QMessageBox::critical(NULL, "Error", QString("目标指数或代码为空"));
        return;
    }
    ...
    if (QMessageBox::Yes == QMessageBox::warning(this, "确认信息", info, QMessageBox::Yes|QMessageBox::No))
    {
        ...
        for (int i = 0; i < timeTypeList.size();)
        {
            ...
            QWidget* chartView = new CSSChartForm(m_chartViews.size(), dbhost, currTimeTypeList,
                                                startDate, endDate, selectCode, hedgedCode,
            ...
            chartView->setWindowTitle(QString("%1,[%2, %3], %4 指标图")
                                     .arg(selectCode).arg(startDate).arg(endDate).arg(currTimeTypeList.join(", ")));
...
void Widget::on_historyEnergyChart_clicked()
{
    if (m_portfolioMap.size () == 0)
    {
        QMessageBox::critical(NULL, "Error", QString("还未选择策略"));
    ...
    else
    {
        ...
        if (QMessageBox::Yes == QMessageBox::warning(this, "确认信息", info, QMessageBox::Yes|QMessageBox::No))
        {
            ...
            for (int i = 0; i < timeTypeList.size();)
            {
                ...
                chartView->setWindowTitle(QString("%1, [%2, %3], %4 指标图")
                                         .arg(m_portfolioFileName).arg(startDate).arg(endDate)
                                         .arg(currTimeTypeList.join(", ")));
