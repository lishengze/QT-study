    ...
    if (tmpFile.exists()) {
        ...
        for (int i = 0; i < oriAccountData.size(); ++i) {
            if (oriAccountData[i].size() < 2) {
                QMessageBox::critical(NULL, "Error", QString::fromLocal8Bit("读取失败: 账户持仓组合: %1, 内容有错误").arg(fileName));
                m_currAccountMap.clear();
                return;
            }
        ...
        updateProgramInfo(ui->programInfo_Table, QString::fromLocal8Bit("读取成功: 账户持仓组合 %1, ").arg(m_currAccountFileInfo.fileName()));
    }
...
void GeneratePortfolioForm::on_geneBuySalePortfolio_clicked()
{
    ...
    if (m_currAccountMap.size() == 0) {
        ...
        QString info = QString::fromLocal8Bit("选择的持仓组合为: ") + m_currAccountFileInfo.fileName() + "\n"
...
QList<QString> GeneratePortfolioForm::getAccountStringInfo(QString accountFileName) {
    ...
    if (currAccountFileInfoList.size() >= 3) {
        ...
        currAccountName = currAccountFileInfoList[0];
        currAccoutTimeInfo = "";
    }
...
void GeneratePortfolioForm::storeBuySalePortfolio() {
    ...
    QList<QString> tmpAccountInfo = getAccountStringInfo(m_currAccountFileInfo.fileName());
