...
RealTimeDataWrite::RealTimeDataWrite(QList<QString> secodeList,
                                     QTableView* programInfoTableView, QTableView* errorMsgTableView,
                                     QObject* parent):
...
void RealTimeDataWrite::initDatabase() {
    m_realtimeDatabase = new RealTimeDatabase("0", m_dbName);
    checkDatabase();
}
...
void RealTimeDataWrite::createSubWriteThreads(QList<QMap<QString, QStringList>> data) {
    ...
    for (int i = 0; i < data.size(); ++i) {
        SubWriteClass* subWriteObj = new SubWriteClass(data[i], QString("%1").arg(i +  m_writeRealTimeDataCount * (m_subWriteThreadNumb + 1)),
                                                       m_dbName, m_dbHost);
